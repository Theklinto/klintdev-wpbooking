...
class BlockedDurationListView extends PartialPage {
	...
	public static function render(): string|false {
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
			<?php foreach ( $blockedDurations as $blockedDuration ) { ?>
                <tr>
					...
					<?= Table::editButton(
						MenuHandler::getInstance()->HiddenEditBlockedDuration->getUrl( [
							"id" => $blockedDuration->getPropertyValue( BlockedDurationListRequest::ID_INT )
						] )
					) ?>
                </tr>
			<?php } ?>
			<?= Table::noRowsText( "Der er ingen låste perioder", $blockedDurations ) ?>
            </tbody>
        </table>
